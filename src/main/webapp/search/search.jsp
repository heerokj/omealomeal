...
//out.println("한페이지당 게시물건수 recordperpage=>:"+recordPerPage);
int startNo = (currentPage-1)*recordPerPage;
//out.println("게시물 시작번호startNo=>:"+startNo);
int endNo = currentPage*recordPerPage;
//out.println("게시물끝번호endNo=>:"+endNo);

//StoreInfoDAO dao = new StoreInfoDAO(); 
//ArrayList<StoreInfoVO> pagelist = dao.selectAll(startNo, endNo);

	String category = request.getParameter("category");
	
	ArrayList<StoreInfoVO> list = null;
	if(category != null) {
		list = dao.select(category, "HITS", 0);
	}else{
	  	list = dao.select("강동", "HITS", 0);
	}
...
<script>
	...
	for (int i = 0; i < idex ; i++) { %>
		...
		geocoder.addressSearch('<%=list.get(i).getAddrs()%>',
						...
						var infowindow = new kakao.maps.InfoWindow(
								{
									content : '<div style="width:300px;text-align:center;padding:6px 3px;padding-bottom:10px;">'+
												'<span style="color:navy;font-weight:bold;"><%=list.get(i).getStoreName()%></span>'+
												'<span style="color:grey;">  <%=list.get(i).getCategory()%></span>'+
												'<p style="font-size:12px;text-align:left;margin-top:3px;"><%=list.get(i).getAddrs()%></p></div>'
							});
...
@import
   ...
   *{
    ...
    #sort{
        margin: 0 auto;
        height: 106px;
        position:relative; /*관리자로그인시 정렬버튼 안밀리게 */
    }
     ...
     .btn{
        ...
        position:absolute; /*관리자로그인시 정렬버튼 안밀리게 */
        top:80px;
        left:490px;
    } 
	/* 정렬(select)하기 버튼 */
...
<div id="container">
	...
	<form action="searchOk.jsp" name="frm">
       ...
       <!------------------------- 정렬버튼 -------------------------->
                ...
                if (mvo.getGrade() == 1){
                %>
                <input class="btn" type="button" value="가게등록" onclick="storePlus();">
                <%} 
                %>
                    <select name="order" id="select_value">
                        <option value="AVGSTAR">별점순</option>
                        <option value="HITS" selected>조회순</option>
                        <option value="REVIEW">리뷰갯수순</option>
                    </select>
                </div>
             ...
             <!-------- 체크박스 필터표현 -------->
                    ...
                    <div id="area"> <!-- 체크박스(지역) -->
                        <table>
                            ...
                            <tr>
                                <td colspan="2"><input type="checkbox" name="district" id="" value="종로 중구 용산">  종로/중구/용산</td>
                            </tr>
                            <tr>
                                <td colspan="2"><input type="checkbox" name="district" id="" value="서초 강남 송파 강동">  서초/강남/송파/강동</td>
                            </tr>
                            <tr>
                                <td colspan="2"><input type="checkbox" name="district" id="" value="성북 강북 노원 도봉">  성북/강북/노원/도봉</td>
                            </tr>
                            <tr>
                                <td colspan="2"><input type="checkbox" name="district" id="" value="동대문 중랑 성동 광진">  동대문/중랑/성동/광진</td>
                            </tr>
                            <tr>
                                <td colspan="2"><input type="checkbox" name="district" id="" value="은평 서대문 마포">  은평/서대문/마포</td>
                            </tr>
                            <tr>
                                <td colspan="2"><input type="checkbox" name="district" id="" value="강서 양천 구로">  강서/양천/구로</td>
                            </tr>
                            <tr>
                                <td colspan="2"><input type="checkbox" name="district" id="" value="영등포 동작 관악 금천">  영등포/동작/관악/금천</td>
                            </tr>
                        </table>
                    </div> <!-- 체크박스 지역영역 end -->
