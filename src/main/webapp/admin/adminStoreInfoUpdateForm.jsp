...
StoreInfoVO storeVo = storeDao.selectOne(storeNum);
%>
<!DOCTYPE html>
<html>
<head>
...
.file>label {
    ...
    position: relative;
	margin: 5px auto;
    background-image: url('<%=storeVo.getStoreImage()%>');
...
<div id="container">
		...
		<form action="storeInfoUpdateOK.jsp" method="post" enctype="multipart/form-data">
		<input type="hidden" name="storeNum" value="<%=storeVo.getStoreNum()%>" /> 
		<%-- <p class="hide"><%=storeVo.getStoreName() %><input type="button" value="" id="edit" /> </p> --%>
		<div class="editform">
			<p class="warn">수정할 내용만 입력해주세요.</p>
			<div id="storeName">
			<h3>상호명</h3>
			<input type="text" name="storeName" value="<%=storeVo.getStoreName() %>"/>
			</div>
			<h3>주소</h3>	
			<input type="text" name="addrs1" id="" placeholder="도로명 주소를 입력해주세요" value= "<%=storeVo.getAddrs() %>" readonly/>
			<input type="button" value="도로명주소" id="search" /><br>
			<input type="text" name="addrs2" id="" placeholder="상세 주소를 입력해주세요" />
			<input type="hidden" name="district" value="<%=storeVo.getDistrict()%>"/>
			<input type="hidden" name="simpleaddrs" value="<%=storeVo.getSimpleAddrs()%>"/>
			<h3>전화번호</h3>	
			<input type="text" name="tel" id="" placeholder="가게 전화번호를 입력해주세요" value="<%=storeVo.getTel()%>"/>
			<h3>업종</h3>	
			<select name="category" id="">
				...
				for(int i=0; i<categoryname.length; i++){
					if(categoryname[i].equals(storeVo.getCategory())){ %>
						<option value="<%=categoryname[i]%>" selected="selected"><%=categoryname[i]%></option>
					<%} else { %>
			...
			<h3>주메뉴</h3>	
			<input type="text" name="storeMenu" id="" placeholder="주 메뉴를 입력해주세요" value="<%=storeVo.getStoreMenu()%>"/>
			<h3>예약 가능 좌석</h3>	
			<input type="text" name="acceptRez" id="" placeholder="예약가능한 좌석을 입력해주세요" value="<%=storeVo.getAcceptRez()%>"/>
			<div class="select">
			<h3>영업시간</h3>
			<select name="opentime" id="">
				...
				for(int i=0; i<48; i++){
					if(i%2 == 0){ 
						if(storeVo.getOpentime().equals((i/2)+":00")){ %>
							<option selected="selected" value="<%=i/2%>:00"><%=i/2%>:00</option>
						<%} else {%>
							<option value="<%=i/2%>:00"><%=i/2%>:00</option>
						<%}
					} else{ 
						if(storeVo.getOpentime().equals((i/2)+":30")){ %>
							<option selected="selected" value="<%=i/2%>:30"><%=i/2%>:30</option>
						<%} else { %>
			...
			<select name="closetime" id="">
				...
				for(int i=0; i<48; i++){
					if(i%2 == 0){ 
						if(storeVo.getClosetime().equals((i/2)+":00")){ %>
							<option selected="selected" value="<%=i/2%>:00"><%=i/2%>:00</option>
						<%} else {%>
							<option value="<%=i/2%>:00"><%=i/2%>:00</option>
						<%}
					} else{ 
						if(storeVo.getClosetime().equals((i/2)+":30")){ %>
							<option selected="selected" value="<%=i/2%>:30"><%=i/2%>:30</option>
						<%} else { %>
							<option value="<%=i/2%>:30"><%=i/2%>:30</option>
						<%
			...
			<div class="file">
				...
				<input type="hidden" name="img" value="<%=storeVo.getStoreImage() %>" />
			</div>
			</div>
		</div>
